...
- `src/meeting-files/` — CQRS, same shape as `src/meetings/`, a separate module (per the PRD) that never imports `MeetingsModule` directly — it fetches meetings via `QueryBus.execute(new GetMeetingByIdQuery(...))` and checks access itself. `MeetingFilesModule` imports `CqrsModule` and `AuthModule`. `MeetingFilesController` is guarded by `JwtAuthGuard` at the class level, routed at `meetings/:meetingId/files`.
  ...
  - `util/decode-original-filename.ts` — `decodeOriginalFilename()` re-interprets `file.originalname` as UTF-8; busboy (multer's underlying parser) decodes multipart header values as latin1 by default, so a non-ASCII file name sent by a real browser comes out as mojibake unless corrected. Applied in the upload `fileFilter` (which runs, and mutates the shared `file` object, before `diskStorage`'s `filename()` callback and the command handler both see it), so the corrected name is what ends up both in the generated storage filename's extension lookup and in the `MeetingFile.name` column.
  - `util/build-content-disposition.ts` — `buildAttachmentDisposition()` builds the download response's `Content-Disposition` header with both a sanitized ASCII `filename=` fallback and an RFC 5987 `filename*=UTF-8''<percent-encoded>` value, so non-ASCII/space-containing file names round-trip correctly instead of most clients saving the file under a literal percent-encoded name (the failure mode of naively putting `encodeURIComponent(name)` inside a plain quoted `filename=`).
